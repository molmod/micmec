...
"""Generate the **fcu** type for further use in a micromechanical system."""
...
def main():
    ...
    f = h5py.File("../atomic/md/output_fcu_0MPa.h5", "r")
    mass = get_mass(f)
    cell = get_cell0(f, start=2000)
    elasticity = get_elasticity0(f, start=2000)
    f.close()

    type_fcu = build_type(material="UiO-66(Zr)", 
                            mass=mass, 
                            cell0=cell,
                            elasticity0=elasticity,
                            topology="fcu")

    with open("type_fcu_new.pickle", "wb") as f:
        pkl.dump(type_fcu, f)
