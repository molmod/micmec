...
__all__ = ["DOF", "CartesianDOF", "BaseCellDOF", "FullCellDOF", "StrainCellDOF"]
...
class BaseCellDOF(DOF):
    ...
    def __init__(
        ...
        if freemask is not None:
            if not (
                isinstance(freemask, np.ndarray)
                and issubclass(freemask.dtype.type, np.bool_)
                and len(freemask.shape) == 1
                and freemask.sum() > 0
            ):
    ...
    def _init_initial(self):
        ...
        if self.freemask is not None and len(self.freemask) != self.ncellvar:
            raise TypeError(
                "The length of the freemask vector (%i) does not "
    ...
    def fun(self, x, do_gradient=False):
        ...
        if do_gradient:
            ...
            self._grvecs[:] = np.dot(U, np.dot(U.T, self._grvecs.ravel())).reshape(
                -1, 3
            )
    ...
    def check_convergence(self):
        ...
        if not self.do_frozen and self.th_gpos_rms is not None:
            ...
            conv_vals.append(
                (
                    self.gpos_max / (self.th_gpos_rms * 3),
                    "gpos_max(%i)" % self.gpos_indmax,
                )
            )
        if self.th_dpos_rms is not None:
            conv_vals.append((self.dpos_rms / self.th_dpos_rms, "dpos_rms"))
    ...
    def log(self):
        ...
        for i in range(len(rvecs)):
            log(
                ...
                % (
                    rvec_names[i],
                    log.length(rvecs[i, 0]),
                    log.length(rvecs[i, 1]),
                    log.length(rvecs[i, 2]),
                )
            )
        log(" ")
