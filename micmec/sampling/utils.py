...
def get_random_vel(temp0, scalevel0, masses, select=None):
    ...
    vel0 = np.random.normal(0, 1, shapevel0) * np.sqrt(
        boltzmann * temp0 / masses
    ).reshape(-1, 1)
...
def clean_momenta(pos, vel, masses, domain):
    ...
    masses :
        ...
        DTYPE: float
        The nodal masses.
    domain : micmec.pes.ext.Domain object
...
def inertia_tensor(pos, masses):
    ...
    itens :
        ...
        The inertia tensor.
    """
    itens = np.identity(3) * (masses.reshape(-1, 1) * pos**2).sum() - np.dot(
        pos.T, masses.reshape(-1, 1) * pos
    )
    return itens
...
def domain_lower(rvecs):
    ...
    C = rvecs[2]
    assert (
        np.dot(np.cross(A, B), C) > 0
    ), "Domain vectors should form right-handed basis!"
    # Vector a.
    ...
    new_rvecs[2, 1] = (np.dot(B, C) - new_rvecs[1, 0] * new_rvecs[2, 0]) / new_rvecs[
        1, 1
    ]
    new_rvecs[2, 2] = np.sqrt(
        np.dot(C, C) - new_rvecs[2, 0] ** 2 - new_rvecs[2, 1] ** 2
    )
