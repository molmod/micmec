...
class Application(tk.Tk):
    ...
    def event_load(self):
        ...
        filename = filedialog.askopenfilename(
            ...
            filetypes=(("CHECKPOINT FILES", "*.chk"), ("ALL FILES", "*.*")),
        )
...
class MenuBar(tk.Menu):
    ...
    def __init__(self, manager):
        ...
        menu_help.add_command(
            label="Tutorial", command=lambda: manager.event_show(TutorialPage)
        )
        menu_help.add_command(
            label="About", command=lambda: manager.event_show(AboutPage)
        )
...
class Page(tk.Frame):
    ...
    def __init__(self, manager):
        super(Page, self).__init__(manager, height=Page._height, width=Page._width)
...
class TutorialPage(Page):
    def __init__(self, manager):
        ...
        self.img1 = tk.PhotoImage(
            ...
            + "/tutorial_images/builder1.png"
        )
        self.img2 = tk.PhotoImage(
            file=os.path.dirname(os.path.realpath(__file__))
            + "/tutorial_images/builder2.png"
        )
        self.img3 = tk.PhotoImage(
            file=os.path.dirname(os.path.realpath(__file__))
            + "/tutorial_images/builder3.png"
        )
        self.img4 = tk.PhotoImage(
            file=os.path.dirname(os.path.realpath(__file__))
            + "/tutorial_images/builder4.png"
        )
        self.img5 = tk.PhotoImage(
            file=os.path.dirname(os.path.realpath(__file__))
            + "/tutorial_images/builder5.png"
        )
        self.img6 = tk.PhotoImage(
            file=os.path.dirname(os.path.realpath(__file__))
            + "/tutorial_images/builder6.png"
        )
...
class DataWidget(Widget):
    ...
    def __init__(self, master):
        ...
        self.pack(side="left", expand="yes", fill="both", padx=10, pady=10)
        self.data = {}
...
class DataWidgetButtons(ttk.Frame):
    ...
    def select_add_data(self):
        ...
        filepaths = filedialog.askopenfilenames(
            title="Select Data", initialdir=os.getcwd(), filetypes=filetypes
        )
        if len(filepaths) == 0:
            return None
...
class DataWidgetTreeview(ttk.Frame):
    ...
    def __init__(self, widget):
        ...
        self.view = ttk.Treeview(
            self, show="headings", columns=DataWidgetTreeview._columns, height=30
        )
        ...
        self.hscroll = ttk.Scrollbar(self, orient="horizontal", command=self.view.xview)
...
class BuilderWidget(Widget):
    ...
    def update_colors_types(self):
        ...
        for key in keys_to_remove:
            self.grid = np.where(self.grid == key, int(0), self.grid)
...
class BuilderWidgetSelector(ttk.Frame):
    def __init__(self, widget):
        ...
        self.combobox_types = ttk.Combobox(
            self, postcommand=self.update_list, state="readonly"
        )
        ...
        self.spinbox_layers = ttk.Spinbox(
            self,
            from_=1,
            to=self.widget.nz,
            state="readonly",
            command=self.select_layer,
            width=5,
        )
...
class BuilderWidgetButtons(ttk.Frame):
    def __init__(self, widget):
        ...
        self.check_x = ttk.Checkbutton(
            self, variable=self.pbc_x
        )  # style="new.TCheckbutton"
        ...
        self.spinbox_nx = ttk.Spinbox(
            self, from_=1, to=20, state="readonly", command=self.update_nx, width=5
        )
        self.spinbox_ny = ttk.Spinbox(
            self, from_=1, to=20, state="readonly", command=self.update_ny, width=5
        )
        self.spinbox_nz = ttk.Spinbox(
            self, from_=1, to=20, state="readonly", command=self.update_nz, width=5
        )
